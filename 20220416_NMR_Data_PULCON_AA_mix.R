#--import data
library(readxl)
peaks_df <- read_excel("~/BPS1/RP1/NMR_data/Quantified_data/2022/20220416_NMR_Data_PULCON_AA_Mix.xlsx")

#--mean of each sample  
means_vector <- rowMeans(peaks_df[, 3:11],na.rm = TRUE) # create vector with mean of each row 
...
library(matrixStats)
concentrations_matrix <- data.matrix(means_df[, 3:11], rownames.force = NA) # convert df into matrix
...
amines_df <- all_samples_df[, -c(3:11)] # 20 amines df 
