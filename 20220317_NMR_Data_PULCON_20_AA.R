...
#--import data
library(readxl)
peaks_df <- read_excel("~/BPS1/RP1/NMR_data/Quantified_data/20220317_NMR_Data_PULCON_20_AA.xlsx")

#--mean of each sample  
means_vector <- rowMeans(peaks_df[, 4:11],na.rm = TRUE) # create vector with mean of each row 
...
library(matrixStats)
concentrations_matrix <- data.matrix(means_df[, 4:11], rownames.force = NA) # convert df into matrix
...
amines_df <- all_samples_df[, -c(1,4:11)] # 20 amines df 
