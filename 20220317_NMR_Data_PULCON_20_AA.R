...
#--mean of each sample  
means_vector <- rowMeans(peaks_df[, 3:10],na.rm = TRUE) # create vector with mean of each row 
...
library(matrixStats)
concentrations_matrix <- data.matrix(means_df[, 3:10], rownames.force = NA) # convert df into matrix
...
amines_df <- all_samples_df[, -c(3:10)] # 20 amines df 
